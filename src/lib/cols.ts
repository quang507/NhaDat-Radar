...
// Bản cho trang CHI TIẾT đọc bằng client thường (khách chưa đăng nhập = anon): KHÔNG có contact_phone.
// Migration 026 thu quyền anon đọc cột này; select nó bằng anon là PostgREST trả 42501 cho CẢ truy vấn.
// SĐT thật chỉ tải riêng khi đã đăng nhập (xem listings/[id]/page.tsx).
export const LISTING_PUBLIC_COLS = "id,source,source_site,source_url,source_post_id,agent_id,project_id,deal,kind,title,description,price_vnd,area_m2,price_per_m2,bedrooms,bathrooms,floors,direction,legal_status,furnishing,province,district,ward,address,lat,lng,amenities,specs,images,contact_name,phone_masked,ai_score,trust_score,poster_role_guess,poster_listing_count,poster_reasons,poster_key,price_flag,source_count,source_sites,status,posted_at,first_seen_at,last_seen_at,crawled_at,crawl_count,created_at";
